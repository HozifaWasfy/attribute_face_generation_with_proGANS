...
def get_loader(image_size):
    ...
    dataset = CelebA_dataset("../celeba-dataset/img_align_celeba/img_align_celeba", "17_attributes.csv", transform=transform)
...
def train_fn(
    ...
    for batch_idx, (real, labels) in enumerate(loop):
        ...
        labels = labels.view((cur_batch_size,config.ATTRIB_DIM))
        ...
        with torch.cuda.amp.autocast():
            ...
            gp = gradient_penalty(critic, real,labels, fake, alpha, step, device=config.DEVICE)
            loss_critic = (
        ...
        if batch_idx % 500 == 0:
            with torch.no_grad():
                fixed_fakes = gen(config.FIXED_NOISE,config.FIXED_LABELS, alpha, step) * 0.5 + 0.5
            plot_to_tensorboard(
                writer,
                loss_critic.item(),
                loss_gen.item(),
